...
optimizer_aux = optim.Adam(auxillary.parameters(), lr = 1e-4)
...
for epoch in range(10000):
    for i, (data,_) in enumerate(train_loader):
        ...
        fake = auxillary(mu.detach(),logvar.detach())
        labelv = Variable(label.fill_(fake_label))
        output = netD(fake.detach())
        errD_fake = criterion(output, labelv)
        errD_fake.backward()

        x_aux = auxillary(mu.detach(),logvar.detach())
        out_aux = netD(x_aux.detach())
        L_aux = criterion(out_aux, labelv)
        L_aux.backward()
        
        D_G_z1 = output.data.mean()
        errD = errD_real + errD_fake + L_aux
        optimizerD.step()

        #step dummy
        
        x1 = auxillary(mu.detach(),logvar.detach())
        out1 = netD(x1)
        labelv = Variable(label.fill_(real_label))
        LGAN1 = criterion(out1,labelv)
        out_orig = netD(inputv)
        L_aux = loss_function(out1,out_orig.detach(),mu.detach(),logvar.detach()) + LGAN1
        ...
        if epoch%1==0:
            ...
            labelv = Variable(label.fill_(real_label))
            output_G = netD(fake)
            output_orig_G = netD(inputv)
            #errG = criterion(output,labelv) + loss_function(fake,inputv,mu,logvar)
            errG = loss_function(output_G,output_orig_G,mu,logvar) #+ 0.5*criterion(output,labelv)
        ...
        if(epoch %1 ==0):
            ...
            if i % 100 == 0:
                ...
                vutils.save_image(x1.data.view(-1,1,28,28),
                                  '%s/aux_samples.png' % (args.outf),
                                  normalize=True)
